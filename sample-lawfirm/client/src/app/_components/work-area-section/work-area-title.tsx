...
export default function WorkAreaTitle({
  ...
  const getIcon = (Icon: React.ElementType) => {
    return (
      <Icon className='w-4 h-4 md:w-6 md:h-6 lg:w-[30px] lg:h-[30px] text-[var(--mb-icon)] md:text-[var(--dt-icon)] transition-colors duration-200' />
    );
  };
  ...
  return (
    <div className='inline-block' style={dynamicStyles}>
      ...
      <div
        className={`font-bold text-[16px] md:text-[28px] lg:text-[40px] text-[var(--mb-text)] md:text-[var(--dt-text)] transition-colors duration-200`}
      >
        {title}
      </div>
    </div>
